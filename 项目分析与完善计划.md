...
**涉及文件：**
- `mapper/resetPasswordMapper.java` + `resetByUsername..xml`（双点文件名）
- `service/resetPasswordSerivce.java`（类名拼写错误）

**现状：**
Service 层有逻辑但存在致命 Bug，没有 Controller 对外暴露。

**存在的问题：**

| 问题 | 严重程度 | 说明 |
|------|---------|------|
| 正则表达式完全错误 | 致命 | `identifier.matches("[*@.*]")` 只能匹配单个字符，永远无法匹配邮箱地址 |
| Mapper 参数绑定不匹配 | 致命 | XML 中 `#{Username}` 与 Java 方法参数名 `newPassword` 不对应 |
| 方法返回 String 类型 | 高 | UPDATE 操作应返回 int（影响行数），String 会导致 MyBatis 转换异常 |
| 没有传 identifier 给 SQL | 致命 | WHERE 条件的值永远是 null |
| 没有 Controller | 致命 | 功能无法对外使用 |
| XML 中 SQL 末尾有分号 | 中 | 部分数据库驱动会报错 |
...
在完善之前，以下接口基本可用（需确认数据库连接正常）：

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/hello` | 健康检查，返回 "Hello Dream Archive" |
| POST | `/api/register` | 用户注册（密码 BCrypt 加密） |
| POST | `/api/analysisDream` | 存储梦境（不含 AI 解析） |
| GET | `/api/dream/{id}` | 按 ID 查询梦境 |
